...
describe RSpecStripe::Runner do
  ...
  describe "invoice" do
    context "without a customer" do
      it "raises error" do
        runner = RSpecStripe::Runner.new({invoice: true})
        expect { runner.call! }.to raise_error("No customer given")
      end
    end
    ...
    context "with a customer" do
      ...
      let(:invoice_double) { double(Stripe::Invoice, id: "invoice-id") }
      ...
      before(:each) {
        ...
        expect(Stripe::Invoice).to receive(:list).once.with(expected_params) { { data: [invoice_double] } }
      }
